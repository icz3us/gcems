...
<div
    class="gc-card rounded-2xl hover:-translate-y-1 transition-all duration-300 overflow-hidden flex flex-col h-full relative group">
    ...
    <div class="p-5 flex flex-col flex-grow relative">
        ...
        <div class="flex justify-between items-start mb-4 gap-3 pr-4">
            ...
            <span
                class="inline-flex items-center rounded-full px-2.5 py-1 text-[11px] font-black uppercase {{ $statusColor }} shrink-0">
                {{ $statusText }}
            </span>
        </div>
        ...
        <div class="mt-auto">
            @if(isset($event))
                <a href="{{ route('events.show', $event) }}"
                    class="block w-full rounded-xl px-4 py-3 text-center text-sm font-black transition-colors focus:outline-none focus:ring-2 focus:ring-offset-2 {{ $isCancelled ? 'bg-gray-100 text-gray-500 hover:bg-gray-200 focus:ring-gray-500' : 'bg-white text-[#007a34] border border-green-200 hover:bg-green-50 focus:ring-green-500' }}">
                    View Details
                </a>
            @else
                <button
                    class="w-full py-2.5 px-4 rounded-lg font-medium transition-colors focus:outline-none focus:ring-2 focus:ring-offset-2 {{ $isFull ? 'bg-gray-100 text-gray-500 cursor-not-allowed' : 'bg-white text-green-700 border border-green-200 hover:bg-green-50 focus:ring-green-500' }}"
                    {{ $isFull ? 'disabled' : '' }}>
